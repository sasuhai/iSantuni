...
export default function AddProgramPage() {
    ...
    const [lookups, setLookups] = useState({
        states: [],
        kawasan: [],
        subKategori: [], // This will now store objects {name, groups}
        anjuran: [],
        statusOptions: [],
        kategoriOptions: []
    });
    ...
    useEffect(() => {
        const fetchLookups = async () => {
            setLoading(true);
            try {
                const { data: statesData } = await supabase.from('states').select('name').order('name');
                const { data: kawasanData } = await supabase.from('kawasan_cawangan').select('name, state_name');
                ...
                const organizers = (orgData && orgData.length > 0) ? orgData.map(o => o.name) : ['Staf Negeri', 'Wilayah', 'HQ', 'IKRAM', 'GDM', 'MAIN / JAIN', 'Lain-lain'];

                const kawasanObj = {};
                if (kawasanData) {
                    kawasanData.forEach(k => {
                        if (!kawasanObj[k.state_name]) kawasanObj[k.state_name] = [];
                        kawasanObj[k.state_name].push(k.name);
                    });
                }
                ...
                setError("Ralat memuatkan data pilihan.");
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Optional: Remove empty strings from time fields so they don't cause DB type errors if optional
            // Ensure arrays are handled
            const { mod_program, ...rest } = formData;
            const payload = {
                ...rest,
                // If DB expects text for jenis_program, join it, but here we assume it might be array
                // For safety, we can join with commas if the column is text
            ...
            console.error(err);
            setError(err.message || "Ralat semasa menyimpan rekod program. Sila cuba lagi.");
            setSubmitting(false);
        }
    };
                    ...
                    <div className="mb-8 flex flex-col items-start gap-4">
                        ...
                        <div>
                            ...
                            <h1 className="text-3xl font-extrabold text-[#1E293B] tracking-tight">
                                Tambah <span className="text-emerald-500">Program / Aktiviti</span>
                            </h1>
                        </div>
                    </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                    ...
                                    <div>
                                        ...
                                        <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 max-h-48 overflow-y-auto space-y-2">
                                            {(!formData.negeri || lookups.kawasan[formData.negeri]?.length === 0) && <span className="text-xs text-slate-400 italic">Sila pilih negeri dahulu atau tiada cawangan dijumpai</span>}
                                            {formData.negeri && lookups.kawasan[formData.negeri]?.map(k => (
                                                <label key={k} className="flex items-center group cursor-pointer">
                                                    <div className="relative flex items-center justify-center">
                                                        <input
                                                            type="checkbox"
                                                            checked={formData.kawasan_cawangan.includes(k)}
                                                            onChange={() => handleCheckboxListChange('kawasan_cawangan', k)}
                                                    ...
                                                    <span className="ml-3 text-sm font-medium text-slate-700 group-hover:text-emerald-600 transition-colors">{k}</span>
                                                </label>
                                            ))}
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden flex flex-col">
                                        ...
                                        <div className="max-h-64 overflow-y-auto divide-y divide-slate-100">
                                            {(() => {
                                                ...
                                                if (filteredItems.length === 0) return <div className="p-8 text-center text-xs text-slate-400 italic">Tiada jenis program dijumpai</div>;
