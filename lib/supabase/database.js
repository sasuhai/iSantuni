...
export const createRateCategory = async (data) => {
    try {
        await apiFetch('lookup', {
            method: 'POST',
            body: JSON.stringify({ table: 'rateCategories', name: data.kategori, extraData: { jenis: data.jenis } }),
        });
        return { error: null };
    } catch (error) {
        return { error: error.message };
...
export const updateRateCategory = async (id, data) => {
    try {
        await apiFetch(`lookup?table=rateCategories&id=${id}`, {
            method: 'PUT',
            body: JSON.stringify({ kategori: data.kategori, jenis: data.jenis }),
        });
        return { error: null };
    } catch (error) {
        return { error: error.message };
...
export const fetchAll = async (supabaseQuery) => {
    try {
...
export const getScoreboardStats = async () => {
    try {
